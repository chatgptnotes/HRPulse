...
export default function TablePagination({ total, page, pageSize, onPageChange, noun = 'rows' }: Props) {
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-4 border-t-2 border-slate-200 px-5 py-4 text-xs text-slate-600 bg-gradient-to-r from-slate-50 to-white">
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className="rounded-lg border border-slate-300 bg-white px-3 py-2 font-semibold text-slate-700 disabled:opacity-40 disabled:cursor-not-allowed hover:border-purple-400 hover:bg-purple-50 transition-all shadow-sm disabled:shadow-none"
        >
          <span className="material-icons text-lg">chevron_left</span>
        </button>
        <span className="rounded-xl bg-gradient-to-r from-purple-500 to-indigo-600 px-4 py-2 text-white font-bold shadow-md shadow-purple-500/30">{page}</span>
        <span className="font-semibold text-slate-500">of {totalPages}</span>
        <button
          type="button"
