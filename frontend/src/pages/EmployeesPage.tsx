...
export default function EmployeesPage() {
  ...
  const { data: salaryLedgers = [], isLoading: ledgersLoading, refetch: refetchSalaryLedgers } = useQuery<SalaryLedger[]>({
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['employees'] }); qc.invalidateQueries({ queryKey: ['salary-ledgers'] }); setEditing(null); setLedgerSearch(''); },
  ...
  const matchingLedgers = salaryLedgers.filter(ledger =>
    !ledgerSearchText || `${ledger.accountName} ${ledger.accountCode} ${ledger.accountId || ''}`.toLowerCase().includes(ledgerSearchText)
  ).slice(0, 12);
  ...
  function openEdit(emp: Employee) {
    ...
    const selectedLedger = emp.ledgerId == null ? undefined : salaryLedgers.find(ledger => ledger.id === emp.ledgerId);
    setLedgerSearch(selectedLedger?.accountName || emp.ledgerName || emp.accountCode || '');
    setLedgerFocused(false);
    setForm({
  ...
  useEffect(() => {
    if (!editing || !form.salaryLedgerId || ledgerSearch.trim()) return;
    const selectedLedger = salaryLedgers.find(ledger => ledger.id === form.salaryLedgerId);
    if (selectedLedger) setLedgerSearch(selectedLedger.accountName);
  }, [editing, form.salaryLedgerId, ledgerSearch, salaryLedgers]);

  function directoryDisplayName(emp: Employee) {
    return [emp.ledgerName, emp.name, emp.biometricName].find(value => String(value || '').trim()) || '—';
  }
          ...
          <div className="flex max-h-[calc(100dvh-5rem)] w-[calc(100vw-3rem)] max-w-[320px] flex-col overflow-hidden rounded-xl bg-white shadow-2xl sm:max-h-[calc(100vh-2rem)] sm:w-full sm:max-w-2xl sm:rounded-2xl">
            ...
            <div className="overflow-y-auto px-4 py-4 sm:px-6 sm:py-5">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div className="relative"><label className="mb-1.5 block text-xs font-medium text-slate-700">Adamrit Salary Ledger</label><div className="relative"><input value={ledgerSearch} onFocus={() => { setLedgerFocused(true); refetchSalaryLedgers(); }} onBlur={() => setTimeout(() => setLedgerFocused(false), 120)} onChange={e => { setLedgerFocused(true); setLedgerSearch(e.target.value); setForm(prev => ({ ...prev, salaryLedgerId: null })); }} placeholder={ledgersLoading ? 'Loading Adamrit ledgers...' : 'Search ledger name or Account ID'} className="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5 pr-9 text-sm outline-none focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100" />{ledgerSearch && <button type="button" onClick={() => { setLedgerSearch(''); setLedgerFocused(false); setForm(prev => ({ ...prev, salaryLedgerId: null })); }} className="absolute right-2 top-1/2 -translate-y-1/2 rounded p-1 text-slate-400 hover:bg-slate-200" aria-label="Clear salary ledger"><span className="material-icons text-sm">close</span></button>}</div>{form.salaryLedgerId && (() => { const selected = salaryLedgers.find(ledger => ledger.id === form.salaryLedgerId); return selected ? <p className="mt-1 text-[11px] font-medium text-emerald-700">Linked Adamrit Account ID: {selected.accountId || selected.accountCode}</p> : null; })()}{ledgerFocused && ledgerSearch.trim() && !ledgersLoading && <div className="absolute left-0 right-0 top-full z-20 mt-1 max-h-64 overflow-y-auto rounded-lg border border-slate-200 bg-white p-1 shadow-xl">{matchingLedgers.length ? matchingLedgers.map(ledger => <button type="button" key={ledger.id} onMouseDown={event => { event.preventDefault(); setForm(prev => ({ ...prev, salaryLedgerId: ledger.id })); setLedgerSearch(ledger.accountName); setLedgerFocused(false); }} className="flex w-full flex-col items-start rounded-md px-3 py-2 text-left hover:bg-indigo-50"><span className="w-full whitespace-normal break-words text-xs font-semibold leading-5 text-slate-700">{ledger.accountName}</span><span className="mt-0.5 text-[10px] text-slate-400">Account ID: {ledger.accountId || ledger.accountCode}</span></button>) : <p className="px-3 py-2 text-xs text-slate-400">No Adamrit ledger matches.</p>}</div>}<p className="mt-1 text-[10px] text-slate-400">Live data from Adamrit chart_of_accounts. Search by ledger name or Account ID.</p></div>
