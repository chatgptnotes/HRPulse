...
export default function SalaryPage() {
  ...
  const printSalarySheet = (company: string) => {
    ...
    const body = printable.map(({ emp, config: cfg, deduction: ded }) => {
      ...
      const lop = ded ? Number(ded.lopDays || 0) * salary / 30 : 0;
    ...
    <div className="w-full min-w-0 p-4 sm:p-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Set basic salary in Indian Rupees (₹) to calculate Loss of Pay</p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        ...
        <table className="w-full min-w-0 table-fixed text-[10px] sm:text-[11px]">
          <colgroup>
            <col className="w-[22%]" />
            <col className="w-[13%]" />
            <col className="w-[9.285%]" />
            <col className="w-[9.285%]" />
            <col className="w-[9.285%]" />
            <col className="w-[9.285%]" />
            <col className="w-[9.285%]" />
          ...
          <tbody>
            {filteredRows.map(({ emp, config: cfg, deduction: ded, hasAttendance, hasSalary }) => {
              ...
              const liveLopAmount = ded && Number(currentSalary) > 0 ? (Number(ded.lopDays || 0) * Number(currentSalary)) / 30 : null;
              const netPayable = Number(currentSalary) > 0 ? Math.max(0, Number(currentSalary) - (liveLopAmount || 0)) : null;
              return (
                <tr key={emp.id} className="border-b border-slate-100 hover:bg-slate-50">
                  ...
                  <td className="px-0.5 py-2.5">
                    <input
                      ...
                      className="w-full max-w-[105px] border border-slate-200 rounded-lg px-1 py-1.5 text-[11px] focus:outline-none focus:ring-2 focus:ring-brand-500/30"
                    />
                  ...
                  <td className="px-0.5 py-2.5 text-center font-medium text-red-600">
                    {liveLopAmount !== null ? formatINR(liveLopAmount) : Number(currentSalary) > 0 ? '₹0' : '—'}
                  </td>
                  <td className="px-0.5 py-2.5 text-center font-semibold text-emerald-700">
